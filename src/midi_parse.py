...
Updated to work with Python 3 standards in February 2014 by Matthew Louvier
"""
...
def showstr(str_, n=16):
    for x in str_[:n]:
        print('{0:x}  '.format(ord(x)))
    print()

def getNumber(str_, length):
    # MIDI uses big-endian for everything
    sum_ = 0
    for i in range(length):
        sum_ = (sum_ << 8) + str_[i]
    return sum_, str_[length:]
...
attrib))
    ...
    def read(self, time, str_):
        global runningStatus
        self.time = time
        # do we need to use running status?
        if not (str_[0] & 0x80):
            str_ = runningStatus + str_
        runningStatus = x = str_[0]
        y = x & 0xF0
        z = str_[1]
        ...
        if channelVoiceMessages.has_value(y):
            ...
            length, str_ = getVariableLengthNumber(str_[2:])
            self.data = str_[:length]
            return str_[length:]
    ...
    def write(self):
        ...
        if channelVoiceMessages.hasattr(self.type):
            ...
            str_ = chr(sysex_event_dict[self.type])
            str_ = str_ + putVariableLengthNumber(len(self.data))
            return str_ + self.data

        elif metaEvents.hasattr(self.type):
            str_ = chr(0xFF) + chr(getattr(metaEvents, self.type))
            str_ = str_ + putVariableLengthNumber(len(self.data))
            return str_ + self.data
...
class DeltaTime(MidiEvent):
    ...
    def write(self):
        str_ = putVariableLengthNumber(self.time)
        return str_
...
class MidiTrack:
    ...
    def read(self, str_):
        time = 0
        assert str_[:4] == b"MTrk"
        length, str_ = getNumber(str_[4:], 4)
        self.length = length
        mystr = str_[:length]
        remainder = str_[length:]
        while mystr:
    ...
    def write(self):
        ...
        for e in self.events:
            str_ = str_ + e.write()
        return "MTrk" + putNumber(len(str_), 4) + str_
...
class MidiFile:
    ...
    def readstr(self, str_):
        assert str_[:4] == b"MThd"
        length, str_ = getNumber(str_[4:], 4)
        assert length == 6
        format_, str_ = getNumber(str_, 2)
        self.format = format_
        assert format_ == 0 or format_ == 1   # dunno how to handle 2
        numTracks, str_ = getNumber(str_, 2)
        division, str_ = getNumber(str_, 2)
        if division & 0x8000:
    ...
    def writestr(self):
        ...
        str_ = "MThd" + putNumber(6, 4) + putNumber(self.format, 2)
        str_ = str_ + putNumber(len(self.tracks), 2)
        str_ = str_ + putNumber(division, 2)
...
def main(argv):
    global debugflag
    import getopt
    infile = "D:\Matt's Stuff\Extra Music\Midi & Musicwrite\Angry Beavers.mid"
    outfile = 'example2.mid'
    printflag = 1
